<template>
    <div>
        <div class="row ">
            <div class="col-md-12">
                <h2 class="pageTitle">Student Batch Manager</h2>
            </div>
                    ...
                    <div class="row p-4">
                        ...
                        <div class="col-md-8">
                            <ow-data-table :headers="tableHeader"
                                           :tableHeader="'Student Batch List'"
                                           :suggestText="'Batch Record list on table. Filter Batch using the filter.'"
                                           :url="url"
                                           :noDataMessage="'No Batch data found. Please Batch filter to show.'"
                                           :hasSearch="true"
                                           :has-multiple="true"
                                           :has-pagination="true"
                                           :main-item="mainItem"
    ...
    export default {
        ...
        data() {
            return {
                searchData: {},
                tableHeader: [
                    {name: 'Student Batch', sort_key: 'title'},
                    {name: 'Status'},
                    {name: 'Action'},
                ],
                title: '',
        ...
        methods: {
            ...
            changeStatus(id, status) {
                ...
                let url = '/json/student-batch/' + id + '/' + stat;
            ...
            editItems(id) {
                this.$router.push({name: 'batchEdit', params: {id: id}})
            },
            deletePopModal(id) {
                this.deleteItem = id;
                this.deletePop = true
            ...
            deleteItems() {
                this.$http.get('/json/student-batch' + this.deleteItem + '/delete').then(res => {
                    this.getData();
                    this.$root.notification.status = res.data[0];
                    this.$root.notification.message = res.data[1]
                })
