                ...
                @if (isset($data['student']) && $data['student']->count() > 0)
                    ...
                    @foreach($data['student'] as $student)
                        <tr>
                            ...
                            <td style="display: inline-flex; float: center;">
                                    <a href="{{ route($base_route.'.view', ['id' => $student->id]) }}" class=" btn-primary btn-sm mr-1">
                                        <i class="ace-icon fa fa-eye bigger-130"></i>
                                    </a>

                                    <a href="{{ route($base_route.'.edit', ['id' => $student->id]) }}" class="btn-success btn-sm mr-1">
                                        <i class="ace-icon fa fa-pencil bigger-130"></i>
                                    </a>

                                    <a href="{{ route($base_route.'.delete', ['id' => $student->id]) }}" class="btn-danger btn-sm mr-1 bootbox-confirm" >
                                        <i class="ace-icon fa fa-trash-o bigger-130"></i>
                                    </a>
                                </td>
                            <td>
                                <div class="hidden-sm hidden-xs action-buttons">
                                    <a href="{{ route('library.member.quick-membership', ['reg_no' => $student->reg_no,'user_type' => 1,'status' => 'active',]) }}" class="label label-primary btn-minier">
