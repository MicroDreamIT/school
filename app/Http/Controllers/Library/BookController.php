...
class BookController extends CollegeBaseController
{
    ...
    public function update(EditValidation $request, $id)
    {
        ...
        $request->merge(['image' => isset($image_name) ? $image_name : ""]);
    ...
    public function bookStatus(request $request, $id, $status)
    {
        ...
        $request->merge(['book_status' => $status]);

        $row->update($request->all());

        return response()->json(['success', $row->faculty . ' ' . $this->panel . ' Status Change Successfully.']);
    }
    ...
    public function addCopies(Request $request)
    {
        ...
        if ($start == 0 or $end == 0) {
            return response()->json(['warning', $this->panel . ' Attention!, Please Enter Start Value Greater Than 0']);
        }

        if ($start > $end) {
            return response()->json(['warning', $this->panel . ' Attention!, Yo have enter End Value is Less than Start. Correct It.']);
        }
        ...
        if ($quantity > 0) {
            ...
            while ($i <= $quantity) {
                ...
                if ($row) {
                    return response()->json(['warning', $this->panel . ' Code Already Exist Please Correct and Try Again.']);
                } else {
                    Book::create([
        ...
        return response()->json(['success', $this->panel . ' Copies Add Successfully.']);
    ...
    public function bulkCopiesDelete(Request $request)
    {
        if ($request->has('bulk_action') && in_array($request->get('bulk_action'), ['delete'])) {
            if ($request->has('chkIds')) {
                ...
                return response()->json(['warning', 'Please, Check at least One Book.']);
            }
